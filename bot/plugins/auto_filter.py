...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("📋Tap to Next Page📋", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"⚔️Check Links at @srsuggestionsmc⚔️..Found {(len_results)} Results For Your Request 💠: <code>{query}</code>",
